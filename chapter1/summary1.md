...
5. > 命题的否定也可以看做 **否定运算符**作用在命题上的结果。
   ...
   |$F$|$T$|
6. > 逻辑运算符也称为 **联结词** 。
7. > 命题 $p$ **且** $q$ ，写作 $p \wedge q$ 。
8. > 命题 $p$ **或** $q$ ，写作 $p \vee q$ 。
9. > 命题 $p$ **同或** $q$，写作 $p \bigotimes q$ 。
10. > 命题 $p$ **异或** $q$，写作 $p \bigoplus q$ 。
11. 命题 **合取** 的真值表
    |$p$|$q$|$p \wedge q$|
    |----|----|----|
    |$T$|$T$|$T$|
...
2. >**布尔搜索** ：采用命题逻辑的技术搜索。
3. >在**布尔搜索** 中，联结词 *$AND$* 用于匹配同时包含两个搜索项的记录，联结词 *$OR$* 用于匹配两个搜索项之一或两项均匹配的记录，而联结词 *$NOT(ND NOT)$* 用于排除某个特定的搜索项。
...
6. >*平凡证明** ：通过证明 $q$ 为真，可以推出 $p \rightarrow q$ 一定为真，用 $q$ 为真的额事实来证明 $p \rightarrow q$ 的方法叫做平凡证明。
