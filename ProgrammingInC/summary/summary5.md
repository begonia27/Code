...
1. >当程序执行遇到 *$continue$* 语句时，该语句后面的所有语句都将被跳过，循环的其他部分仍将照常执行。
2. >*$continue$* 语句最长江的用途是：当满足某些条件的时候，跳过循环体中的部分语句，反之则执行这些语句。
3. >*$continue$* 的格式如下： *$continue;$*
4. >作为初学者，除非对于 *$break$* 语句和 *$continue$* 语句非常熟悉，否则读者应该避免在程序中使用 *$break$* 语句和 *$continue$* 语句。这两个语句很容易被滥用，导致编写出来的程序很难阅读和调试。
