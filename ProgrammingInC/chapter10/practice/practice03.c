...
/* 接收一个字符类型的参数（用ascll码表来判断），看是否属于大小写字母和包含撇号的单词，
...
{
    int i, number, wordCount = 0;

    // 对变量lookingForWord进行声明和初始化，对调用的函数进行声明
    bool lookingForWord = true, alphabetic(const char c);
    bool lookingForSymbol = true, isSpecialSymbol(int number);

    for (i = 0; string[i] != '\0'; ++i)
    {
        // 确认是字母或者特殊的符号
        if (alphabetic(string[i]) || isSpecialSymbol(string[i]))
        {
            // 并且属于一个单词和特殊的符号
            if (lookingForWord && lookingForSymbol)
            {
                // 将变量的计数增加1
                ++wordCount;
